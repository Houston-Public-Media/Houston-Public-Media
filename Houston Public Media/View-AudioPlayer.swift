			...
			HStack {
				if playback.audioType == .stream {
					...
					AsyncImage(url: URL(string: playback.currentEpisode?.image.full.url ?? "")) { image in
						image.resizable().aspectRatio(contentMode: .fit).frame(width: 30, height: 30).cornerRadius(8)
					} placeholder: {
						ProgressView()
					}
				}
		...
		default:
			HStack {
				if playback.audioType == .stream {
					...
					AsyncImage(url: URL(string: playback.currentEpisode?.image.full.url ?? "")) { image in
						image.resizable().aspectRatio(contentMode: .fit).frame(width: 35, height: 35).cornerRadius(8)
					} placeholder: {
						ProgressView()
					}
				}
